...
const GodetteTexture = preload("res://addons/localization_editor_plugin_g4/godette.png")
const GodetteWowTexture = preload("res://addons/localization_editor_plugin_g4/godette_wow.png")

const LinkBtnFile = preload("res://addons/localization_editor_plugin_g4/LinkButtonRecentFile.tscn")
const TranslationItem = preload("res://addons/localization_editor_plugin_g4/HBxItemTranslation.tscn")

var Locales = preload("res://addons/localization_editor_plugin_g4/localization_locale_list.gd").new()
...
func _ready() -> void:
	...
	if Engine.is_editor_hint() == false:
		DisplayServer.window_set_mode(Conf.get_value("main","maximized", DisplayServer.WINDOW_MODE_WINDOWED))
...
func load_recent_files_list() -> void:
	...
	for rl in recent_list:
		var Btn: LinkButtonRecentFile = LinkBtnFile.instance()
		Btn.f_path = rl
		Btn.opened.connect(self._OnRecentFile_opened)
		Btn.removed.connect(self._OnRecentFile_removed)
...
func _on_BtnNewFileCreate_pressed() -> void:
	...
	if F:
		...
		OS.alert("Error creating file. Error #"+str(FileAccess.get_open_error()))
		F.close()
...
func _on_Dock_resized() -> void:
	if Engine.is_editor_hint() == false:
		Conf.set_value("main","maximized",DisplayServer.window_get_mode())
		Conf.save(_self_data_folder_path+"/translation_manager_conf.ini")
